    ...
    public static class InvoiceService
    {
            ...
            int? voucherId = null)
        {
            ...
            try
            {
                ...
                if (voucherId.HasValue) VoucherService.UpdateVoucherUsage(voucherId.Value, connection, tx);
        ...
        private static int ResolveEmployeeIdFromCurrentUserOrDefault()
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(currentUser)) return UserService.GetEmployeeIdByUsername(currentUser);
            }
            catch { }
            return 1;
        }
        ...
        private static bool SaveInvoiceWithResolvedCustomer(InvoiceHeader header, List<(int ProductId, int Quantity, decimal UnitPrice)> items, int defaultEmployeeId)
        {
            try
            {
                var customerId = CustomerService.GetOrCreateCustomerId(header.CustomerName, header.CustomerPhone, header.CustomerEmail, header.CustomerAddress);
