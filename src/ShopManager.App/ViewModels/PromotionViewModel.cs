...
namespace ShopManager.App.ViewModels
{
    public class PromotionViewModel : BaseViewModel
    {
        ...
        private async System.Threading.Tasks.Task LoadDataAsync()
        {
            ...
            var catsTask = System.Threading.Tasks.Task.Run(() => CategoryService.GetAllCategories());
            var promosTask = System.Threading.Tasks.Task.Run(() => PromotionService.GetAllPromotions());

            await System.Threading.Tasks.Task.WhenAll(productsTask, catsTask, promosTask);
            ...
            Categories.Clear();
            foreach (var c in catsTask.Result)
            {
                Categories.Add(new CategoryViewModel { Id = c.Id, Name = c.Name });
            }

            Promotions.Clear();
            foreach (var p in promosTask.Result)
            {
                Promotions.Add(p);
            }
        }
