...
namespace ShopManager.Core.Settings
{
    public class SettingsConfig
    {
        public string Server { get; set; } = "localhost";
        public uint Port { get; set; } = 26803;
    ...
    public static class SettingsManager
    {
        private static readonly string SettingsPath = Path.Combine(
            ...
            "ShopManager", "settings.json");

        private static readonly byte[] OptionalEntropy = Encoding.UTF8.GetBytes("ShopManagerEntropy");
        ...
        public static bool Save(SettingsConfig config, out string error)
        {
            try
            {
                ...
                error = string.Empty;
                lock (_cacheLock) { _cachedConnectionString = null; }
                return true;
            }
            catch (Exception ex)
            {
