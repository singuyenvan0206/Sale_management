...
namespace ShopManager.Web.Controllers
{
    ...
    {
        private readonly ShopManager.Services.ICustomerService _customerService;
        private readonly ShopManager.Core.Interfaces.ISystemSettingsService _settingsService;

        public SettingsController(ShopManager.Services.ICustomerService customerService, ShopManager.Core.Interfaces.ISystemSettingsService settingsService)
        {
        ...
        {
            var settings = ShopManager.Core.Settings.PaymentSettingsManager.Load();
            settings.SePayToken = await _settingsService.GetSePayTokenAsync() ?? "";
            return View(settings);
        }

        [HttpPost]
        public async Task<IActionResult> Payment(ShopManager.Core.Settings.PaymentSettings settings)
        {
            if (ModelState.IsValid)
            {
                // Save other settings to file (legacy)
                ShopManager.Core.Settings.PaymentSettingsManager.Save(settings);
