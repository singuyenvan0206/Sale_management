...
namespace ShopManager.App.ViewModels
{
    public class SettingsViewModel : BaseViewModel
    {
        ...
        public bool UseApiMode { get => _useApiMode; set => SetProperty(ref _useApiMode, value); }
        ...
        {
            var cfg = SettingsManager.Load();
            UseApiMode = cfg.UseApiMode;
            ApiUrl = string.IsNullOrWhiteSpace(cfg.ApiUrl) ? "http://localhost:5000" : cfg.ApiUrl;
            ApiToken = cfg.ApiToken;
        }

        public async Task TestApiConnectionAsync()
        {
            StatusText = "Đang kiểm tra kết nối REST API Server...";
            ApiClient.SetBaseUrl(ApiUrl);
            var res = await ApiClient.GetAsync<object>("swagger/v1/swagger.json");
            if (res.Success || res.Message.Contains("200"))
            {
                StatusText = "Kết nối Web API Server thành công!";
                MessageBox.Show("Kết nối tới REST API Server thành công!", "Thành công", MessageBoxButton.OK, MessageBoxImage.Information);
            }
            else
            {
                StatusText = $"Kết nối API thất bại: {res.Message}";
                MessageBox.Show($"Kết nối API thất bại.\nChi tiết: {res.Message}", "Lỗi kết nối API", MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
        ...
        public void SaveApiSettings()
        {
            ...
            if (SettingsManager.Save(cfg, out string error))
            {
                ...
                StatusText = "Đã lưu cài đặt REST API!";
                MessageBox.Show("Đã lưu cấu hình REST API thành công!", "Thành công", MessageBoxButton.OK, MessageBoxImage.Information);
            }
            else
            {
                MessageBox.Show($"Không thể lưu cấu hình API: {error}", "Lỗi", MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
        ...
        {
            var cfg = new SettingsConfig { Server = Server, Port = Port, Database = Database, UserId = UserId, Password = password };
