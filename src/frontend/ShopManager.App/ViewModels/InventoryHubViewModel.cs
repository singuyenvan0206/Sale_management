...
namespace ShopManager.App.ViewModels
{
    public class InventoryHubViewModel : BaseViewModel
    {
        ...
        private void LoadData() => _ = LoadDataAsync();

        private async System.Threading.Tasks.Task LoadDataAsync()
        {
            try
            {
                var suppliersTask = System.Threading.Tasks.Task.Run(() => SupplierService.GetAllSuppliers());
                var productsTask = System.Threading.Tasks.Task.Run(() => ProductService.GetAllProductsWithCategories());
                var posTask = System.Threading.Tasks.Task.Run(() => PurchaseOrderService.GetAllOrders());
                var movementsTask = System.Threading.Tasks.Task.Run(() => StockMovementService.GetLatestMovements(100));

                await System.Threading.Tasks.Task.WhenAll(suppliersTask, productsTask, posTask, movementsTask);

                Suppliers.Clear();
                foreach (var s in suppliersTask.Result) Suppliers.Add(s);
                ...
                foreach (var po in posTask.Result) PurchaseOrders.Add(po);

                StockMovements.Clear();
                foreach (var m in movementsTask.Result) StockMovements.Add(m);
            }
            catch (Exception ex)
            {
                AppLogger.Log(ex, "InventoryHubViewModel.LoadDataAsync");
            }
        }
        ...
        private void LoadPODetails(int poId)
        {
            System.Threading.Tasks.Task.Run(() =>
            {
                try
                {
                    var details = PurchaseOrderService.GetOrderById(poId);
                    Application.Current.Dispatcher.Invoke(() => SelectedPODetails = details);
                }
                catch (Exception ex)
                {
        ...
        private void CreatePO(object? obj)
        {
            ...
            var po = new PurchaseOrder
            {
                ...
                PaidAmount = 0,
                Status = ShopManager.Core.Enums.PurchaseOrderStatus.Pending.ToDbString(),
                Notes = PONotes,
