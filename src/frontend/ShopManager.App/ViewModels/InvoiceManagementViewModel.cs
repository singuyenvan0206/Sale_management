...
namespace ShopManager.App.ViewModels
{
    public class InvoiceManagementViewModel : BaseViewModel
    {
        ...
        public List<string> DiscountModes { get; } = new() { ShopManager.Core.Enums.VoucherDiscountType.FixedAmount.ToDbString(), ShopManager.Core.Enums.VoucherDiscountType.Percentage.ToDbString() };
        ...
        private void OpenHistory()
        {
            try
            {
                ...
                var historyWin = new ShopManager.App.Views.TransactionHistoryWindow(SelectedCustomer?.Id);
                
                // Try to set owner to the current dashboard if possible
                if (Application.Current.MainWindow is ShopManager.App.Views.DashboardWindow dashboard)
                {
