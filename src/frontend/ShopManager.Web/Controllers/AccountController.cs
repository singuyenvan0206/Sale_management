        ...
        public async Task<IActionResult> Login(string username, string password, string? returnUrl = null)
        {
            ...
            if (loginResult.IsSuccess && loginResult.Value)
            {
                ...
                var claims = new List<Claim>
                {
                    ...
                    new Claim(ClaimTypes.Role, roleResult.Value ?? ShopManager.Core.Models.UserRole.Cashier.ToString()),
            ...
            _logger.LogWarning("Failed login attempt for username '{Username}' from {IP}. Error: {Error}",
                username, HttpContext.Connection.RemoteIpAddress, loginResult.ErrorMessage);
            ModelState.AddModelError(string.Empty, loginResult.ErrorMessage ?? "Tên đăng nhập hoặc mật khẩu không chính xác.");
            return View();
        }
