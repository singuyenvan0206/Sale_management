...
namespace ShopManager.Core.Models
{
    ...
    {
        // Global Settings
        public decimal SpendPerPoint { get; set; } = 10000;
        ...
        {
            var parsed = CustomerTypeExtensions.ParseCustomerType(tier);
            return parsed switch
            {
                CustomerType.Silver => SilverDiscountPercent,
                CustomerType.Gold => GoldDiscountPercent,
                CustomerType.VIP => VIPDiscountPercent,
        ...
        public string GetBenefitsForTier(string tier)
        {
            ...
            return parsed switch
            {
                CustomerType.Silver => SilverBenefits,
                CustomerType.Gold => GoldBenefits,
                CustomerType.VIP => VIPBenefits,
                _ => RegularBenefits
            };
        }
        ...
        public string GetDescriptionForTier(string tier)
        {
            ...
            return parsed switch
            {
                CustomerType.Silver => SilverDescription,
                CustomerType.Gold => GoldDescription,
                CustomerType.VIP => VIPDescription,
        ...
        {
            if (points >= VIPMinPoints) return CustomerType.VIP.ToDbString();
            if (points >= GoldMinPoints) return CustomerType.Gold.ToDbString();
            if (points >= SilverMinPoints) return CustomerType.Silver.ToDbString();
            return CustomerType.Regular.ToDbString();
        }
    }
    ...
    public static class TierSettingsManager
    {
        private static readonly string SettingsPath = Path.Combine(
            ...
            "ShopManager", "tier_settings.json");
