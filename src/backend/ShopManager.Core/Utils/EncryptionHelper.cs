...
namespace ShopManager.Core.Utils
{
    public static class EncryptionHelper
    {
        ...
        {
            var envKey = Environment.GetEnvironmentVariable("SHOP_MANAGER_AES_KEY");
        ...
        {
            var envIv = Environment.GetEnvironmentVariable("SHOP_MANAGER_AES_IV");
            if (!string.IsNullOrEmpty(envIv))
            {
                var bytes = Encoding.UTF8.GetBytes(envIv);
                if (bytes.Length == 16) return bytes;
            }
            return Encoding.UTF8.GetBytes("Sh0pM4n_IV_2026!");
        }
