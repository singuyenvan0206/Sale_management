...
namespace ShopManager.Data
{
    ...
    public static class DatabaseMigration
    {
        ...
        private static void BackfillBarcodes(MySqlConnection connection)
        {
            ...
            {
                Console.WriteLine($"Error backfilling barcodes: {ex.Message}");
            }
        }
        ...
        private static void RandomizeAllBarcodes(MySqlConnection connection)
        {
            try
            {
                ...
                ExecuteNonQuery(connection, "SET SQL_SAFE_UPDATES = 1;");
                Console.WriteLine("Successfully randomized all barcodes in database.");
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Error randomizing barcodes: {ex.Message}");
            }
        }
        ...
        private static void SyncCustomerPointsAndTotalSpent(MySqlConnection connection)
        {
            ...
            {
                Console.WriteLine($"Error syncing customer points: {ex.Message}");
            }
        }
        ...
        private static void FixIncorrectProductCategories(MySqlConnection connection)
        {
            try
            {
                ...
                ExecuteNonQuery(connection, "SET SQL_SAFE_UPDATES = 1;");
                Console.WriteLine("Successfully reorganized all product categories based on keywords.");
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Error fixing product categories: {ex.Message}");
            }
        }
        ...
        private static void MigrateTransactionHistoryView(MySqlConnection connection)
        {
            ...
            {
                Console.WriteLine($"Error migrating TransactionHistory view: {ex.Message}");
            }
        }
        ...
        private static void MigrateAccountsTable(MySqlConnection connection)
        {
            try
            {
                ...
                // Migrate legacy and plaintext passwords to BCrypt hashes
                MigratePlaintextPasswords(connection);
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Error migrating Accounts table: {ex.Message}");
            }
        }

        private static void MigratePlaintextPasswords(MySqlConnection connection)
        {
            try
            {
                var accounts = connection.Query<(int Id, string Username, string Password)>("SELECT Id, Username, Password FROM Accounts");
                foreach (var account in accounts)
                {
                    if (!string.IsNullOrWhiteSpace(account.Password) && !PasswordHelper.IsHashed(account.Password))
                    {
                        string hashedPassword = PasswordHelper.HashPassword(account.Password);
                        connection.Execute("UPDATE Accounts SET Password = @Password WHERE Id = @Id", new { Password = hashedPassword, Id = account.Id });
                        Console.WriteLine($"Auto-hashed plaintext password for user '{account.Username}' during database migration.");
                    }
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Error automatically migrating plaintext passwords: {ex.Message}");
            }
        }
        ...
        private static void MigrateCategoriesTable(MySqlConnection connection)
        {
            ...
            {
                Console.WriteLine($"Error migrating Categories table: {ex.Message}");
            }
        }
        ...
        private static void MigrateSuppliersTable(MySqlConnection connection)
        {
            ...
            {
                Console.WriteLine($"Error migrating Suppliers table: {ex.Message}");
            }
        }
        ...
        private static void MigrateProductsTable(MySqlConnection connection)
        {
            try
            {
                ...
                foreach (var col in unusedColumns)
                {
                    if (ColumnExists(connection, "Products", col))
                    {
                        ...
                        catch (Exception ex) { Console.WriteLine($"Error dropping column {col}: {ex.Message}"); }
                    }
                }
            ...
            {
                Console.WriteLine($"Error migrating Products table: {ex.Message}");
            }
        }
        ...
        private static void MigrateCustomersTable(MySqlConnection connection)
        {
            ...
            {
                Console.WriteLine($"Error migrating Customers table: {ex.Message}");
            }
        }
        ...
        private static void MigrateVouchersTable(MySqlConnection connection)
        {
            ...
            {
                Console.WriteLine($"Error migrating Vouchers table: {ex.Message}");
            }
        }
        ...
        private static void MigrateInvoicesTable(MySqlConnection connection)
        {
            ...
            {
                Console.WriteLine($"Error migrating Invoices table: {ex.Message}");
            }
        }
        ...
        private static void MigrateInvoiceItemsTable(MySqlConnection connection)
        {
            ...
            {
                Console.WriteLine($"Error migrating InvoiceItems table: {ex.Message}");
            }
        }
        ...
        private static void CreateStockMovementsTable(MySqlConnection connection)
        {
            ...
            {
                Console.WriteLine($"Error creating StockMovements table: {ex.Message}");
            }
        }
        ...
        private static void CreateCustomerVoucherUsageTable(MySqlConnection connection)
        {
            ...
            {
                Console.WriteLine($"Error creating CustomerVoucherUsage table: {ex.Message}");
            }
        }
        ...
        private static void CreatePromotionsTable(MySqlConnection connection)
        {
            ...
            {
                Console.WriteLine($"Error creating Promotions table: {ex.Message}");
            }
        }
        ...
        private static void MigratePromotionsTable(MySqlConnection connection)
        {
            ...
            {
                Console.WriteLine($"Error migrating Promotions table: {ex.Message}");
            }
        }
        ...
        private static void CreateProductVariantsTable(MySqlConnection connection)
        {
            try
            {
                ...
                if (count == 0)
                {
                    ...
                    if (migrated > 0)
                    {
                        Console.WriteLine($"Migrated {migrated} products to default variants.");
                    }
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Error creating ProductVariants table: {ex.Message}");
            }
        }
        ...
        private static void CreateErpTables(MySqlConnection connection)
        {
            try
            {
                ...
                Console.WriteLine("ERP tables checked/created.");
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Error creating ERP tables: {ex.Message}");
            }
        }
