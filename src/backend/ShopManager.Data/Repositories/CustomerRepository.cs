...
namespace ShopManager.Data.Repositories
{
    public class CustomerRepository : MySqlRepositoryBase, ICustomerRepository
    {
        ...
        public async Task<int> RefreshAllLoyaltyAsync(decimal spendPerPoint, int silverMin, int goldMin, int vipMin)
        {
            ...
            string sql = @"
                ...
                SET Points = FLOOR(IFNULL(TotalSpent, 0) / @Spend),
                    CustomerType = CASE
                        WHEN FLOOR(IFNULL(TotalSpent, 0) / @Spend) >= @VIP THEN @VIPStr
                        WHEN FLOOR(IFNULL(TotalSpent, 0) / @Spend) >= @Gold THEN @GoldStr
                        WHEN FLOOR(IFNULL(TotalSpent, 0) / @Spend) >= @Silver THEN @SilverStr
                        ELSE @RegularStr
                    END
                WHERE Id > 0;";
            return await connection.ExecuteAsync(sql, new { 
                ...
                Gold = goldMin, 
                Silver = silverMin,
                VIPStr = CustomerType.VIP.ToDbString(),
                GoldStr = CustomerType.Gold.ToDbString(),
                SilverStr = CustomerType.Silver.ToDbString(),
                RegularStr = CustomerType.Regular.ToDbString()
            });
        }
        ...
        public async Task<int> BulkImportCustomersAsync(List<Customer> customers)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < customers.Count; i += batchSize)
                {
                    ...
                    totalInserted += await connection.ExecuteAsync(sql, batch, tx);
                }
        ...
        {
            using var connection = GetConnection();
            string sql = @"SELECT i.Id AS InvoiceId, i.CreatedDate AS CreatedAt, COUNT(ii.Id) AS ItemCount, i.Total
                           FROM Invoices i
            ...
            {
                list.Add((Convert.ToInt32(r.InvoiceId), Convert.ToDateTime(r.CreatedAt), Convert.ToInt32(r.ItemCount), Convert.ToDecimal(r.Total)));
            }
            return list;
        }
    }
