...
namespace ShopManager.Services
{
    public class UserService : IUserService
    {
        ...
        public async Task<Result> RegisterAccountAsync(string username, string employeeName, string password, string? role = null)
        {
            try
            {
                string roleStr = role ?? UserRole.Cashier.ToRoleString();
                string hashedPassword = PasswordHelper.HashPassword(password);
                var success = await _userRepository.AddAsync(username, employeeName, hashedPassword, roleStr);
                
                if (success)
                {
                    Log.Information("User registered successfully: {Username} as {Role}", username, roleStr);
        ...
        public async Task<Result<string>> GetUserRoleAsync(string username)
        {
            try
            {
                ...
                return Result<string>.Success(userFound.Role ?? UserRole.Cashier.ToRoleString());
            }
            catch (Exception ex)
            {
        ...
        public async Task<Result<UserRole>> GetUserRoleEnumAsync(string username)
        {
            ...
            UserRole role = UserRoleExtensions.ParseUserRole(result.Value);
